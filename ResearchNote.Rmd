...
url <- read_csv("/Users/sunyufei/Desktop/tianjinUrl.csv") %>% 
  dplyr::select(url)
airticle <- data_frame(c(1:length(url$url)))
airticle$title <- c(1:length(url$url))
airticle$text <- c(1:length(url$url))
airticle$source <- c(1:length(url$url))
airticle$publishTime<- c(1:length(url$url))
n = 1

for (n in 1:length(url$url)) {
  airticle$title[n] <- as.character(paste0(list(read_html(url$url[n]) %>% 
  html_nodes("#activity-name") %>% 
  html_text())))
  airticle$text[n] <- as.character(paste0(list(read_html(url$url[n]) %>% 
  html_nodes("section p") %>% 
  html_text())))
  airticle$source[n] <- as.character(paste0(list(read_html(url$url[n]) %>% 
  html_nodes("#js_name") %>% 
  html_text())))
  print(n)
}
...
for (a in 1:length(data$data)) {
  data$cityname[a] <- read_html(data$data[a]) %>% 
  html_nodes("p") %>% 
  html_text()
  print(a)
}
